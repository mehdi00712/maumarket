...
saveShopBtn?.addEventListener("click", async () => {
  ...
  if (!businessName) {
    shopMessage.textContent = "Business name is required.";
    return;
  }

  if (!businessPhone) {
    shopMessage.textContent = "Business phone is required.";
    return;
  }
  ...
  shopMessage.textContent = "Saving private business information...";
...
saveItemBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    const itemData = {
      ...
      // Internal fields used by admin, payouts and delivery.
      shopName:
        currentShop?.shopName ||
        shopName?.value.trim() ||
        "MauMarket Seller",

      // Buyers must never see the private seller identity.
    ...
    itemMessage.textContent = getFriendlySellerError(
      error,
  ...
  snapshot.forEach((docSnap) => {
    ...
    div.innerHTML = `
      ...
      <div class="seller-price-breakdown">
        ...
        <p><strong>Buyer sees:</strong> ${formatRs(buyerPrice)}</p><p><strong>Public seller label:</strong> Verified MauMarket Merchant</p>
      </div>
...
if (requestSlotsBtn) {
  requestSlotsBtn.addEventListener("click", async () => {
    ...
    slotMessage.textContent = "Sending request...";
    ...
    try {
      ...
      slotMessage.textContent = getFriendlySellerError(
        error,
